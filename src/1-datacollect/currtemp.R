...
df3 <- df2 %>% group_by(year) %>% summarize(tmu=mean(t), tci05=quantile(t, .05), tci95=quantile(t, .95), dmu=mean(t2coeff * t^2), dci05=quantile(t2coeff * t^2, .05), dci95=quantile(t2coeff * t^2, .95))

subset(df3, year == 2023)
subset(df3, year == 1950)
