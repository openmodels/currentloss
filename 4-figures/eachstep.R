...
ggsave("../results/eachstep-cumul.pdf", width=2.5, height=2.5)
...
ggplot(pdf, aes(year, mu / 1e9, group=factor(slrconf))) +
    ...
    scale_x_continuous(NULL, expand=c(0, 0), limits=c(1959, 2023))
ggsave("../results/eachstep-slr.pdf", width=2.5, height=2.5)
...
for (persist in c(0.08, 0.21)) {
    ...
    ggsave(paste0("../results/eachstep-trade-", persist, ".pdf"), width=2.5, height=2.5)
}
...
for (persist in c(0.08, 0.21)) {
    ...
    ggsave(paste0("../results/eachstep-solow-", persist, "-", trade.method, ".pdf"), width=2.5, height=2.5)
}
