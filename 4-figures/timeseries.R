...
ggplot(allyr2, aes(Year, total, group=ISO, colour=label)) +
    ...
    theme_bw()
ggsave(paste0("../results/timeseries_", do.for.subset, "-", persist, "-", trade.method, ".pdf"), width = 8, height = 4)
...
ggsave(paste0("../results/globaltime-noloess_", do.for.subset, ".pdf"), width = 6.5, height = 4)
...
tryCatch({
    ...
    print("allyr3.pop/gdp.mc fails on Code Ocean.")
})
...
ggsave(paste0("../results/totalbyyear_", do.for.subset, ".pdf"), width = 5, height = 4)
...
ggsave(paste0("../results/totalbyothers_", do.for.subset, ".pdf"), width=6, height=6)
