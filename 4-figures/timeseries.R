...
for (subset in do.for.subset) {
  ...
  allyr2 <- filtered_data %>% group_by(ISO, Year) %>%
    filter(weight.norm > 1e-9) %>%
    dplyr::summarize(
      solow = ifelse(all(is.na(product.chg)), NA, wtd.median(product.chg - totimpact - -tradeloss - -slrloss, weights = weight.norm, normwt = T)),
      total = ifelse(all(is.na(product.chg)), wtd.median(totimpact - tradeloss - slrloss, weights = weight.norm, normwt = T), wtd.median(product.chg, weights = weight.norm, normwt = T)),
  ...
  ggsave(paste0("figures/timeseries_", subset, ".pdf"), width = 8, height = 4)
}
...
for (subset in do.for.subset) {
  ...
  allyr3.pop <- filtered_data %>% group_by(mc, Year) %>%
  ...
  allyr3.gdp <- filtered_data %>% group_by(mc, Year) %>%
  ...
  #   theme_bw() + scale_y_continuous(y_axis_label, labels=scales::percent) +
  #   scale_x_continuous(NULL, expand=c(0, 0)) + scale_colour_discrete("Weighting:")
  # ggsave(paste0("figures/globaltime_", subset, ".pdf"), width=6.5, height=4)
  ...
  ggsave(paste0("figures/globaltime-noloess_", subset, ".pdf"), width=6.5, height=4)
}
...
for (subset in do.for.subset) {
  ...
  pdf <- filtered_data %>%
    ...
    group_by(Year, mc) %>%
    dplyr::summarize(
      totimpact.usd = sum(totimpact.usd, na.rm = TRUE),
      tradeloss.usd = sum(tradeloss.usd, na.rm = TRUE),
      slrloss.usd = sum(slrloss.usd, na.rm = TRUE),
      solow.usd = sum(solow.usd, na.rm = TRUE),
      total.usd = totimpact.usd - tradeloss.usd - slrloss.usd + solow.usd,
      weight2 = sum(weight.norm)
    ) %>%
    group_by(Year) %>%
    dplyr::summarize(
      totimpact.usd = wtd.median(totimpact.usd, weights = weight2, normwt = TRUE),
      tradeloss.usd = wtd.median(tradeloss.usd, weights = weight2, normwt = TRUE),
      slrloss.usd = wtd.median(slrloss.usd, weights = weight2, normwt = TRUE),
      solow.usd = wtd.median(solow.usd, weights = weight2, normwt = TRUE),
      total.usd.25 = wtd.quantile(total.usd, 0.25, weights = weight2, normwt = TRUE),
      total.usd.75 = wtd.quantile(total.usd, 0.75, weights = weight2, normwt = TRUE),
      total.usd = wtd.median(total.usd, weights = weight2, normwt = TRUE)
    )
  ...
  ggplot(pdf2, aes(Year)) +
    ...
    scale_fill_manual(
      NULL,
      breaks = rev(c('totimpact.usd', 'tradeloss.usd', 'solow.usd', 'slrloss.usd')),
      labels = rev(c("Direct Impact", "International Impact", "Capital Impact", "Coastal Impact")),
      values = c("#1b9e77", "#d95f02", "#7570b3", "#e7298a")
    ) +
    theme(legend.position = c(.5, .25))
  
  ggsave(paste0("figures/totalbyyear_", subset, ".pdf"), width = 8, height = 4)
}
