...
df.gdp3b$GDP.2019.est <- df.gdp3b$GDP.2015.est * 106.87654 / 100

slr <- read.csv("data/slrbyadm0-final.csv")
slr2 <- slr %>% left_join(df.gdp3b, by=c('ISO'='Country Code', 'year'='Year')) %>%
    group_by(ISO, year) %>% reframe(mc=1:30, slrloss=rnorm(30, mu / GDP.2019.est, ((q95 - q05) / diff(qnorm(c(.05, .95)))) / GDP.2019.est))
...
allyr.plusslr <- allyr %>% left_join(slr2, by=c('mc', 'Year'='year', 'ISO'))
allyr.plusslr$slrloss[is.na(allyr.plusslr$slrloss) | allyr.plusslr$slrloss < 0] <- 0
allyr.plusslr$slrloss[allyr.plusslr$slrloss > 1] <- 1
allyr.plusslr$prodplusslr <- allyr.plusslr$product.chg - allyr.plusslr$slrloss
allyr.plusslr$itlimpact <- -allyr.plusslr$fracloss
...
allyr.ww <- allyr.plusslr %>% left_join(solowdone3, by=c('ISO', 'mc'), suffix=c('', '.solowdone')) %>%
...
allyr2 <- allyr.ww %>% group_by(ISO, Year) %>%
    ...
    dplyr::summarize(solow=ifelse(rep(all(is.na(prodplusslr)), length(prodplusslr)), NA, wtd.median(prodplusslr - totimpact - itlimpact, weights=weight.norm, normwt=T)), total=ifelse(rep(all(is.na(prodplusslr)), length(prodplusslr)), wtd.median(totimpact + itlimpact, weights=weight.norm, normwt=T), wtd.median(prodplusslr, weights=weight.norm, normwt=T)), prod25=ifelse(rep(all(is.na(prodplusslr)), length(prodplusslr)), wtd.quantile(totimpact + itlimpact, .25, weights=weight.norm, normwt=T), wtd.quantile(prodplusslr, .25, weights=weight.norm, normwt=T)), prod75=ifelse(rep(all(is.na(prodplusslr)), length(prodplusslr)), wtd.quantile(totimpact + itlimpact, .75, weights=weight.norm, normwt=T), wtd.quantile(prodplusslr, .75, weights=weight.norm, normwt=T)), totimpact=wtd.median(totimpact, weights=weight.norm, normwt=T), itlimpact=wtd.median(itlimpact, weights=weight.norm, normwt=T))
...
allyr3.pop <- allyr2 %>% group_by(mc, Year) %>%
    dplyr::summarize(totimpact=wtd.mean(totimpact, weights=POP, normwt=T), itlimpact=wtd.mean(itlimpact, weights=POP, normwt=T), solow=ifelse(all(is.na(prodplusslr)), NA, wtd.mean(prodplusslr - totimpact - itlimpact, weights=POP, normwt=T)), total=ifelse(all(is.na(prodplusslr)), wtd.mean(totimpact + itlimpact, weights=POP, normwt=T), wtd.mean(prodplusslr, weights=POP, normwt=T)), weight2=wtd.mean(weight.norm, weights=POP)) %>%
    group_by(Year) %>%
...
allyr3.gdp <- allyr2 %>% group_by(mc, Year) %>%
    dplyr::summarize(totimpact=wtd.mean(totimpact, weights=GDP.2015, normwt=T), itlimpact=wtd.mean(itlimpact, weights=GDP.2015, normwt=T), solow=ifelse(all(is.na(prodplusslr)), NA, wtd.mean(prodplusslr - totimpact - itlimpact, weights=GDP.2015, normwt=T)), total=ifelse(all(is.na(prodplusslr)), wtd.mean(totimpact + itlimpact, weights=GDP.2015, normwt=T), wtd.mean(prodplusslr, weights=GDP.2015, normwt=T)), weight2=wtd.mean(weight.norm, weights=GDP.2015)) %>%
    group_by(Year) %>%
...
sum((subset(allyr2, Year == 2022) %>% group_by(ISO) %>% dplyr::summarize(GDP.2015=GDP.2015[1]))$GDP.2015, na.rm=T)

## Determine ranges

allyr2 <- allyr.ww %>% group_by(ISO, mc, Year) %>%
    mutate(total=ifelse(all(is.na(prodplusslr)), totimpact + itlimpact, prodplusslr)) %>%
...
pdf <- allyr.ww %>% left_join(polydata, by=c('ISO'='ADM0_A3')) %>%
    ...
    mutate(totimpact.usd=(log2lev(totimpact) / (1 + log2lev(totimpact))) * GDP.2015.est / 1e9,
           ...
           solow=product.chg - totimpact - itlimpact,
...
ggplot(pdf2, aes(Year)) +
    ...
    scale_fill_manual(NULL, breaks=rev(c('totimpact.usd', 'itlimpact.usd', 'solow.usd', 'slrimpact.usd')), labels=rev(c("Direct Impact", "International Impact", "Capital Impact", "Coastal Impact")), values=c("#1b9e77", "#d95f02", "#7570b3", "#e7298a")) + theme(legend.position=c(.5, .25))
